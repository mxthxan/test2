...
def classify(node, sample, metadata):
    if node.answer:
        return node.answer
    attr_index = metadata.index(node.attribute)
    for value, child in node.children:
        if sample[attr_index] == value:
            return classify(child, sample, metadata)
    return None
...
prediction = classify(node, sample, metadata)
print(f"Predicted class for the sample {sample} is: {prediction}")
